...
export function WordGalaxy({
  ...
  useEffect(() => {
    ...
    if (container.current) {
      instance = new WordGalaxyFactory(container.current);
    }
...
export class WordGalaxyFactory {
  ...
  private resetPointerPosition = () => (this.pointerPosition = null);
  ...
  private animate = () => {
    if (!this.pointerPosition) {
      ...
      for (let i = 0; i < this.elements.length; i++) {
        ...
        const force = Math.max(0, 1 - distance / this.distanceScale);

        const moveX = (-dx / distance) * force * this.maxMovement;
        const moveY = (-dy / distance) * force * this.maxMovement;
